...
def define_experiment(exp_dict, args):
    ...
    binaries_to_algs = dict({
            'bin/rundb_YCSB_srivastava_abtree_mcs'          : 'MCS'
          , 'bin/rundb_YCSB_srivastava_abtree_pub'          : 'PubElim'
          , 'bin/rundb_YCSB_bronson_pext_bst_occ'           : 'BCCO10'
          , 'bin/rundb_YCSB_brown_ext_abtree_lf'            : 'LFAB'
          , 'bin/rundb_YCSB_brown_ext_ist_lf'               : 'IST'
          , 'bin/rundb_YCSB_winblad_catree'                 : 'CATree'
          , 'bin/rundb_YCSB_morrison_cbtree'                : 'CBTree'
          , 'bin/rundb_YCSB_natarajan_ext_bst_lf'           : 'NM14'
          , 'bin/rundb_YCSB_wang_openbwtree'                : 'OpenBwTree'
          , 'bin/rundb_YCSB_guerraoui_ext_bst_ticket'       : 'DGT15'
        #   , 'bin/rundb_YCSB_aksenov_splaylist_64'       : 'SplayList' # No reclamation, do not run on large key ranges
    })
    ...
    add_run_param       (exp_dict, 'zeta'               , [0])
    add_run_param       (exp_dict, 'rw_ratio'           , ['-r0.5 -w0.5'])
    ...
    #         ./{binary} -t{thread_count} -s{init_size} -n{max_thread_count} {rw_ratio} -z{zeta} {thread_pinning}
    # '''.replace('{time_cmd}', time_cmd).replace('{max_thread_count}', str(max_thread_count)))

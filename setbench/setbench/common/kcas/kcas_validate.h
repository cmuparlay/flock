...
template <int MAX_K>
void KCASValidate<MAX_K>::rdcssHelp(rdcsstagptr_t tagptr, rdcssptr_t snapshot, bool helpingOther) {
    bool readSuccess;
...
template <int MAX_K>
void KCASValidate<MAX_K>::rdcssHelpOther(rdcsstagptr_t tagptr) {
...
template <int MAX_K>
casword_t KCASValidate<MAX_K>::rdcss(rdcssptr_t ptr, rdcsstagptr_t tagptr) {
    casword_t r;
    do {
...
template <int MAX_K>
void KCASValidate<MAX_K>::helpOther(kcastagptr_t tagptr) {
...
template <int MAX_K>
bool KCASValidate<MAX_K>::help(kcastagptr_t tagptr, kcasptr_t snapshot, bool helpingOther) {
...
template <int MAX_K>
inline casword_t KCASValidate<MAX_K>::readPtr(casword_t volatile *addr) {
    casword_t r;
    do {
        r = rdcssRead(addr);
...
template <int MAX_K>
inline casword_t KCASValidate<MAX_K>::readVal(casword_t volatile *addr) {
    return ((casword_t)readPtr(addr)) >> KCAS_LEFTSHIFT;
}

template <int MAX_K>
inline void KCASValidate<MAX_K>::writeInitPtr(casword_t volatile *addr, casword_t const newval) {
    *addr = newval;
}

template <int MAX_K>
inline void KCASValidate<MAX_K>::writeInitVal(casword_t volatile *addr, casword_t const newval) {
...
template <int MAX_K>
void KCASValidate<MAX_K>::deinitThread() {
    kcas_tid.explicitRelease();
}

template <int MAX_K>
template <typename T>
inline void KCASValidate<MAX_K>::add(casword<T> *caswordptr, T oldVal, T newVal) {
    caswordptr->addToDescriptor(oldVal, newVal);
}
template <int MAX_K>
template <typename T, typename... Args>
void KCASValidate<MAX_K>::add(casword<T> *caswordptr, T oldVal, T newVal, Args... args) {
    caswordptr->addToDescriptor(oldVal, newVal);
    add(args...);
}

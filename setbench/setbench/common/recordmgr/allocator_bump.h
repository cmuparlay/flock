...
//        PAD; // not needed because of padding on end of previous array
        ...
        int bump_memory_bytes_remaining(const int tid) {
            return (((char*) mem[tid*PREFETCH_SIZE_WORDS])+memBytes[2*tid*PREFETCH_SIZE_WORDS]) - ((char*) current[tid*PREFETCH_SIZE_WORDS]);
        }
        bool bump_memory_full(const int tid) {
            return (((char*) current[tid*PREFETCH_SIZE_WORDS])+cachelines*BYTES_IN_CACHE_LINE > ((char*) mem[tid*PREFETCH_SIZE_WORDS])+memBytes[2*tid*PREFETCH_SIZE_WORDS]);
        }
    ...
    public:
        ...
        T* allocate(const int tid) {
            ...
            if (!mem[tid*PREFETCH_SIZE_WORDS] || bump_memory_full(tid)) {
                ...
                MEMORY_STATS {
                    this->debug->addAllocated(tid, memBytes[2*tid*PREFETCH_SIZE_WORDS] / cachelines / BYTES_IN_CACHE_LINE);
                    VERBOSE DEBUG2 {
//                        if ((this->debug->getAllocated(tid) % 2000) == 0) {
//                            this->debugInterfaces->reclaim->debugPrintStatus(tid);
//                            debugPrintStatus(tid);
                            COUTATOMICTID("allocated "<<(memBytes[2*tid*PREFETCH_SIZE_WORDS] / cachelines / BYTES_IN_CACHE_LINE)/*this->debug->getAllocated(tid)*/<<" records of size "<<sizeof(T)<<std::endl);
//                            COUTATOMIC(" ");

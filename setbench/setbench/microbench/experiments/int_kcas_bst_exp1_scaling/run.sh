...
t="30000"
num_trials=1
halved_update_rates="0 5 10 50"
key_range_sizes="20000 200000 2000000"
algorithms="sigouin_int_bst_kcas.alloc_new.reclaim_debra.pool_none.out sigouin_int_bst_kcas.alloc_new.reclaim_debra.pool_none_htm.out"
thread_counts=`cd .. ; ./get_thread_counts.sh`

#t=30000
#num_trials=1
#halved_update_rates="50"
#key_range_sizes="2000000"
#algorithms="int_bst_kcas_unbalanced"
                    ...
                    for alg in $algorithms ; do
                        for n in $thread_counts ; do
                            if ((counting)); then
                                ...
                                f="$exp/step$step.txt"
                                args="-nwork $n -nprefill $n -i $uhalf -d $uhalf -rq 0 -rqsize 1 -k $k -nrq 0 -t $runtime -pin $pinning_policy"
                                cmd="LD_PRELOAD=../../../lib/libjemalloc.so timeout $timeout_s numactl --interleave=all time ../../bin/ubench_${alg} $args"
                                echo "cmd=$cmd" > $f
                                echo "step=$step" >> $f
                                echo "fname=$f" >> $f
